...
def getData(REAL_SONG_IDS=[],data ="datos/Append/ARRAY_ARRAYS_REC.pkl",shuffle=True, seed=100, train=.7):
    ...
    for u in range(len(DATA)):
        ...
        # Crear ejemplos
        USR_TRAIN = np.empty([len(TRAIN_DATA)-1, 3], dtype=np.int32)
        USR_TRAIN[:,0] = u
        USR_TRAIN[:,1] = TRAIN_DATA[:len(TRAIN_DATA)-1]
        USR_TRAIN[:,2] = TRAIN_DATA[1:]

        USR_DEV = np.empty([len(DEV_DATA) - 1, 3], dtype=np.int32)
        USR_DEV[:, 0] = u
        USR_DEV[:, 1] = DEV_DATA[:len(DEV_DATA) - 1]
        USR_DEV[:, 2] = DEV_DATA[1:]

        USR_TEST = np.empty([len(TEST_DATA) - 1, 3], dtype=np.int32)
        USR_TEST[:, 0] = u
        USR_TEST[:, 1] = TEST_DATA[:len(TEST_DATA) - 1]
        USR_TEST[:, 2] = TEST_DATA[1:]
...
def trainNet(X,Y, DX,DY,TX,TY,config, user_size, train=True, save=True):
    ...
    user_emb_size = 128
    user_concat_size = user_emb_size * 2
    song_emb_size = 64
    hidden_size = 32

    profile_size = user_emb_size + song_emb_size
    ...
    with graph.as_default():
        ...
        T0 = tf.Variable(tf.truncated_normal([user_concat_size, user_emb_size], mean=0.0, stddev=1.0 / math.sqrt(hidden_size)), name="T0")
        B0 = tf.Variable(tf.zeros([user_emb_size]), name="B0")

        O2 = tf.Variable(tf.truncated_normal([song_size, song_emb_size], mean=0.0, stddev=1.0 / math.sqrt(song_emb_size)), name="O2")
        BO2 = tf.Variable(tf.zeros([song_emb_size]), name="BO2")
        ...
        #Transformar concat documento de 256 a 128
        ed = tf.matmul(ed,T0)+B0

        # Embedding de cancion
        ec = tf.nn.embedding_lookup(O2, train_dataset[:, 1])
        ec = ec + BO2
        ...
        weights = tf.Variable(tf.truncated_normal([song_size, profile_size], stddev=1.0 / math.sqrt(song_size)),
                              name="weights")
        # softmax biases
...
#Se obtinenen cada uno de los embeddings aprendidos (incluidas las canciones)
D2V_DM_PRESENT  = np.load('embeddings/d2v_dm/present/EMB_MATRIX')
D2V_DBOW_PRESENT = np.load('embeddings/d2v_dbow/present/EMB_MATRIX')

W2V  = np.load('embeddings/w2v/EMB_MATRIX')

#Se concatenan los embeddings de los usuarios para generar el perfil
D2V_CONS = np.concatenate((D2V_DM_COMPLETE,D2V_DBOW_COMPLETE),axis=1) # El consolidado = Consolidado DM + Consolidado DBOW
D2V_PRES = np.concatenate((D2V_DM_PRESENT,D2V_DBOW_PRESENT),axis=1) # El presente = Presente DM + Presente DBOW

D2V_EMB = np.concatenate((D2V_CONS,D2V_PRES),axis=1) # El perfil completo CONS + PRES
...
NOT_EXISTS = list(ALL.difference(REAL))
...
config = {"batch_size": 1024, "learning_rate": 0.0001, "epochs":1000, "seed":100}
